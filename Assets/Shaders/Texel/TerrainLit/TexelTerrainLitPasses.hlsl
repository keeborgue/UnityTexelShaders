...
#include "Assets/Shaders/Texel/TexelFunctions.hlsl"
    ...
    SAMPLER(sampler_TerrainNormalmapTexture) { Filter = MIN_MAG_MIP_POINT; AddressU = Repeat; AddressV = Repeat; };
#endif
...
void InitializeInputData(Varyings IN, half3 normalTS, out InputData input)
{
    ...
    input.positionWS = CalculateSnappedWorldPos(IN.uvMainAndLM.xy, IN.positionWS, _Control_TexelSize);//IN.positionWS;
    half3 SH = half3(0, 0, 0);

#if defined(_NORMALMAP) && !defined(ENABLE_TERRAIN_PERPIXEL_NORMAL)
    half3 viewDirWS = half3(IN.normal.w, IN.tangent.w, IN.bitangent.w);
    viewDirWS = CalculateSnappedWorldViewDir(viewDirWS, input.positionWS);
    ...
    half3 viewDirWS = CalculateSnappedWorldViewDir(IN.viewDir, input.positionWS);

...
Light GetMainLight()
{
    ...
    light.positionWS = _MainLightPosition.xyz;
...
Light GetAdditionalPerObjectLight(int perObjectLightIndex, float3 positionWS)
{
    ...
    light.color = color;
    light.positionWS = lightPositionWS.xyz;
...
half3 DirectBDRF(BRDFData brdfData, half3 lightDirectionWS, float3 positionWS, float3 viewDirectionWS, float3 normalWS)
{
#ifndef _SPECULARHIGHLIGHTS_OFF
    //Recalculate view direction according to recalculated texelized inputData.positionWS
    float3 viewPositionWS = GetCameraPositionWS();
    float3 viewDirectionWSCorrected = viewPositionWS - positionWS;
    ...
    //lightDirectionWS is already correct, as it uses inputData.positionWS during it's calculation process
    float3 halfDir = SafeNormalize(float3(lightDirectionWS) + float3(viewDirectionWSCorrected * viewPositionMult));
...
half3 LightingPhysicallyBased(BRDFData brdfData, half3 lightColor, half3 lightDirectionWS, float3 lightPositionWS, half lightAttenuation, float3 positionWS, float3 viewDirectionWS, float3 normalWS)
{
    half NdotL = saturate(dot(normalWS, lightDirectionWS));
    half3 radiance = lightColor * (lightAttenuation * NdotL);
    return DirectBDRF(brdfData, lightDirectionWS, positionWS, viewDirectionWS, normalWS) * radiance;
}

half3 LightingPhysicallyBased(BRDFData brdfData, Light light, InputData inputData)
{
    return LightingPhysicallyBased(brdfData, light.color, light.direction, light.positionWS, light.distanceAttenuation * light.shadowAttenuation, inputData.positionWS, inputData.viewDirectionWS, inputData.normalWS);
}
    ...
    half smoothness, half occlusion, half3 emission, half alpha)
{
    ...
    color += LightingPhysicallyBased(brdfData, mainLight, inputData);
